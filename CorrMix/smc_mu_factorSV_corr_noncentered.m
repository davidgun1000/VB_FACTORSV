function [particles,w,indx,sir_llh]=smc_mu_factorSV_corr_noncentered(y,B,ft,phi,sig,kapha,N,T,u1,u1_res,num,dim)
...
if num<=dim
   ...
   w=zeros(T,N);
   indx=zeros(T,N);
   particles(:,1:N,1)=sqrt(1/(1-phi^2))*u1(:,t)';
   ...
   for t=2:T
       ...
       particles(:,1:N,t)=phi*(particles(:,indx(t,1:N),t-1))+u1(:,t)';
       logw=-0.5*log(2*pi)-0.5.*log(exp(sig.*particles(1,:,t)+kapha))-0.5.*(1./exp(sig.*particles(1,:,t)+kapha)).*((y(1,t)-B(1,:)*ft(:,t)).^2);
       w(t,:)=exp(logw-max(logw)); 
    ...
    indx=zeros(T,N);
    particles(:,1:N,1)=sqrt(1/(1-phi^2))*u1(:,t)';
    logw=-0.5*log(2*pi)-0.5.*log(sig.*exp(particles(1,:,t)))-0.5.*exp(-(sig.*particles(1,:,t))).*(ft(num-dim,t).^2);
    w(t,:)=exp(logw-max(logw));
    sir_llh=log(mean(w(t,:)))+max(logw);
    w(t,:)=w(t,:)./sum(w(t,:)); 
    for t=2:T
